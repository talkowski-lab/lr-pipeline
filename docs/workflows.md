...
Outputs:
- `bincov_matrix`: Cohort binned-coverage matrix, bgzipped and tabix-indexed.
- `bincov_matrix_idx`: Index for `bincov_matrix`.
- `estimated_copy_numbers`: Per-sample, per-chromosome estimated copy number.
- `binwise_estimated_copy_numbers`: Per-sample, per-`ploidy_bin_size`-bin estimated copy number.
