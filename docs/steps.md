...
8. **[FillFormatFields](../archive/wdl/annotation_utils/FillFormatFields.wdl)** - copied the FORMAT fields across from the `full_vcf` built in step 7, giving `allele_type_annotated_filled_vcf`.
...
| 3 | **[CreateCohortDepthFiles](../archive/wdl/annotation_utils/CreateCohortDepthFiles.wdl)** - bincov matrix, median coverage and ploidy estimates | run, no column | run, no column |
...
| 9 | **[FillFormatFields](../archive/wdl/annotation_utils/FillFormatFields.wdl)** - copied `AD`, `PL`, `GQ`, `DP` and `RNC` for the short variant records across from the regenerated `glnexus_vcf` | `hprc_hgsvc_vcf_V6` | – |
...
| 15 | **[DropGenotypes](../archive/wdl/annotation_utils/DropGenotypes.wdl)** - dropped genotypes to produce a sites-only VCF | – | `aou_sites_vcf` |
...
| Sites-only VCF | Not produced | `aou_sites_vcf` via [DropGenotypes](../archive/wdl/annotation_utils/DropGenotypes.wdl) |
