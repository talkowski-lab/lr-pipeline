...
- The main checkout stays on `main` and is never switched, since every worktree fetches and fast-forwards through it. A Claude Code hook in [`.claude/settings.json`](../.claude/settings.json) enforces this: it denies edits to tracked files there and tells any session that starts there to create a worktree first. Each branch has exactly one worktree; re-running the script with a branch that already exists locally, on the remote, or in a worktree resumes it rather than creating a second one.
