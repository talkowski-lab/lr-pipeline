...
## Branches
- Development should never happen directly on `main`. Every change starts on a branch cut from an up-to-date `origin/main`, named `kj-<kebab-topic>` - e.g. `kj-annotate-dbvar-mem`. [`new_worktree.sh`](../.github/scripts/new_worktree.sh) creates the branch, pushes it, and puts it in its own linked worktree at `../lr-pipeline-worktrees/<branch>`, beside the main checkout, so concurrent sessions never share a working tree. It prints that path, and all work on the branch happens there.
- [`.worktreelinks`](../.worktreelinks) lists the gitignored, local-only paths - currently just `data` - that every worktree shares with the main checkout. The script symlinks each one rather than copying it, so the 141 GB `data/` tree has a single source of truth and edits are visible from every worktree. A listed path that is missing from the main checkout is skipped with a note; one that is not gitignored is an error, since linking it would shadow tracked content. This is deliberately not Claude Code's `.worktreeinclude`, which copies rather than links and only applies to worktrees Claude Code creates itself; `claude --worktree` is not supported here, because its branch naming, the `.dockstore.yml` filters and `merge_branch.sh` all assume the script's layout.
- The main checkout stays on `main` and is never switched, since every worktree fetches and fast-forwards through it. Three layers enforce this, so no single tool has to be trusted to behave. The hook scripts in [`.github/hooks/`](../.github/hooks/) deny edits to tracked files in the main checkout and tell any session that starts there to create a worktree first; they are wired up for Claude Code in [`.claude/settings.json`](../.claude/settings.json) and for Codex in [`.codex/hooks.json`](../.codex/hooks.json), both of which pass the same JSON on stdin and honour the same `permissionDecision: "deny"` reply. Underneath both, [`.githooks/pre-commit`](../.githooks/pre-commit) refuses any commit made in the main checkout, whichever tool or person makes it - enable it once per clone with `git config core.hooksPath .githooks`. Codex additionally requires the project hooks to be trusted once, with `/hooks` in its CLI. Copilot has no hook mechanism, so there it is the `pre-commit` hook and the instructions that apply. Each branch has exactly one worktree; re-running the script with a branch that already exists locally, on the remote, or in a worktree resumes it rather than creating a second one.
- An abandoned branch is dropped from the main checkout with `git worktree remove ../lr-pipeline-worktrees/<branch>` followed by `git branch -d <branch>`; a merged one is cleaned up by the merge script. `git worktree prune` clears the registration left behind when a worktree directory is deleted by hand.
...
- Work on a branch stays uncommitted until it has been reviewed, so that an editor's diff markers keep showing it. Two separate instructions release it, and neither implies the other: "commit" commits the work and pushes the branch, while "merge to `main`" commits anything outstanding and then merges. Keeping the branch current with `main` is not one of them - `git merge origin/main` inside the worktree needs no instruction, and merging rather than rebasing keeps the pushed branch from needing a force push. The branch is merged into `main` following the sequence in [Feature branch test versions](ci-cd.md#feature-branch-test-versions), which removes the branch filter before the merge and removes the worktree and branch after it. Merges are done locally rather than through a pull request, and a rebase or merge conflict is the one point where the merge stops for human review.
