...
# Compare CNVs against their closest reference callset records
library("optparse")
...
out_columns <- c('name','name.1')
if (!is.null(pop_file)) {
    pop=read.table(pop_file)
    pop_colname = paste(pop[,1],'AF',sep='_')
    pop_colname[pop_colname=='ALL_AF']='AF'
    out_columns <- c(out_columns, pop_colname)
}
out_columns <- c(out_columns, 'Reciprocal_Overlap')
...
colnum = ncol(dat)
dat[,ncol(dat)+1] = apply(dat,1, function(x){as.integer(x[colnum])/max(c(abs(as.integer(x[6])), abs(as.integer(x[12]))))})
