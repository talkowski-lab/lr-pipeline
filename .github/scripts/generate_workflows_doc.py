#!/usr/bin/env python3
"""Generate docs/workflows.md from the meta and parameter_meta blocks of each workflow.
...
Section order follows `.dockstore.yml` for the registered workflows, so the document
and the Dockstore registration cannot drift apart. The sub-workflows under `wdl/utils`
are not registered, so their order and grouping headings live in SUBWORKFLOW_GROUPS.
...
Usage: generate_workflows_doc.py [--check | --dry-run]
"""
import argparse
import difflib
import re
...
OUTPUT = REPO_ROOT / "docs" / "workflows.md"
...
def render_workflow(workflow, path):
    """Render one `###` section for a workflow."""
...
def build():
    """Render the whole document, returning its text."""
    workflows, problems = load_workflows()
    if problems:
    ...
    lines = [f"# {TITLE}", INTRO, "", GENERATED_NOTE]
    used = set()
    for title, group, directory in SECTIONS:
        ...
        for index, name in enumerate(names):
            ...
            if index == 0:
                lines.append("")
            lines.extend(render_workflow(workflow, path))
            used.add(name)
...
def main():
    ...
    mode.add_argument("--check", action="store_true", help="fail if docs/workflows.md is not up to date")
    ...
    if args.check:
        ...
        print("\n".join(diff))
        print("\nRun 'python .github/scripts/generate_workflows_doc.py' and commit the result.")
        sys.exit(1)
