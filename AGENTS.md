...
- Never develop directly on `main`. Start every change on a branch cut from an up-to-date `origin/main`, named `kj-<kebab-topic>`, and push it. Rules: [docs/conventions.md](docs/conventions.md#branches).
- When the branch changes a directly-run workflow — or a task library or `wdl/utils/` sub-workflow it imports — add the branch name under `filters.branches` in that workflow's `.dockstore.yml` entry, after `main`, so the branch version is importable into Terra. Only the workflows being tested get the branch; `main` stays listed in every entry.
- Merge to `main` only when explicitly told to. Then run `.github/scripts/merge_branch.sh` from a clean tree and don't ask again — it strips the branch from `.dockstore.yml`, rebases, fast-forwards `main`, pushes and deletes the branch. Merge locally; don't open a pull request. Details: [docs/ci-cd.md](docs/ci-cd.md#feature-branch-test-versions).
- Stop and hand back if the script reports a rebase conflict, leaving the rebase in progress. Conflicts are the one thing to surface for review; never resolve them automatically.
